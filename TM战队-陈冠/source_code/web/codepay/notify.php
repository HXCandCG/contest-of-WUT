...
<script>
	...
	var succeshttp = GetXmlHttpObject();
	var url="/web/successpage.php";
	url = url + "?nickname=" + "<?php echo $user['nickname']?>";
	url = url + "&phone="+ "<?php echo $user['phone']?>";
	url = url + "&location="+ "<?php echo $user['location']?>";
	url = url + "&sroom="+ "<?php echo $user['sroom']?>";
	url = url + "&sid="+ Math.random();
	succeshttp.open("POST",url,false);
	succeshttp.send(null);
	if (succeshttp.readyState==4 || succeshttp.readyState=="complete"){
		if(succeshttp.responseText!="pass"){
			window.alert(succeshttp.responseText);
		}else{
		//setTimeout(function () {
	        //这里可以写一些后续的业务
	        	var mydate= new Date();
				mydate.setMinutes(mydate.getMinutes()+30);
				window.alert("支付成功！预计"+mydate.toLocaleTimeString()+"送达~");
				window.location.href = "/Homepage.php"; //跳转
	        		
	   	//	},  //echo((int)$codepay_config['go_time'] * 1000));//默认3秒后跳转
		}
    }
